...
PCA_Genotype<-plotPCA(vst_dds,intgroup=c("Age"))+labs(title = "PCA of mice of TG mice ages 2 and 8", color = "Group")+coord_fixed(ratio=3)
...
## Dotplot 
dotplot(ego, showCategory=50)+theme(text = element_text(size = 1)) +scale_y_discrete(labels=function(x) str_wrap(x, width=40))+ggtitle('Enriched genes when comparing WT_TG and rTg4510 mice')+ theme(plot.title = element_text(size=16))
...
cnetplot(ego, 
         ...
         cex_label_gene = 0.75,
         shadowtext = "all")+ggtitle('Enriched Genes when comparing rTg4510 and wild-type rTg4510 mice')
...
write.csv(spia_result, file = "spia_result_tg.csv")
```
...
pheatmap(norm_counts_top_40, color=colorRampPalette(c("white", "darkseagreen4", "darkslategrey"))(30), scale="row", cluster_cols = T, show_rownames = T,fontsize = 7,fontsize_row = 4, fontsize_col = 4,labels_row = rownames(dists),annotation_col =annotation_columns,main='Differentially Expressed Genes in WT_TG and rTg4510 mutant mice' )
dev.off()
```
